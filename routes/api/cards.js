...
router.patch("/edit/:id", [auth, member], async (req, res) => {
  try {
    const { title, description, label, selectedDate } = req.body;
    if (title === "") {
...
// Add/Remove a member
router.put(
  "/addMember/:add/:cardId/:userId/:label",
  [auth, member],
  async (req, res) => {
    try {
      ...
      const adminMember = card.members.filter(({isAdmin}) => isAdmin === true).map(({user}) => user)[0];
      const isCardAdmin = adminMember == userId;
