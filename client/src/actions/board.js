...
export const editBoard = (boardId, formData) => async (dispatch) => {

  const { title, description, selectedDate } = formData;

  try {
    const res = await axios.patch(`/api/boards/${boardId}/${title}/${description}/${selectedDate}`, formData, config);
...
export const putComment = (comment) => async (dispatch) => {
  try {
    ...
    const res = await axios.put(`/api/boards/activity/${boardId}`, comment);
...
export const deleteMember = (userId) => async (dispatch) => {
  try {
    const res = await axios.delete(`/api/boards/deleteMember/${userId}`);
